        ...
        if (r.isConfirmed) {
            ...
            google.script.run
                .withSuccessHandler(res => {
                    loading(0);
                    res.success ? Swal.fire('สำเร็จ', res.message, 'success').then(() => softReset()) : alertErr(res.message);
                })
                .withFailureHandler(alertErr)
    ...
    function doCheck() {
       ...
       google.script.run.withSuccessHandler(r=>{
          ...
          if(r.found) {
             ...
             d.innerHTML = `<div class="card shadow-sm"><div class="card-header ${b} text-white fw-bold">${r.status}</div><div class="card-body"><h5>${r.name}</h5><p class="mb-1 text-muted">${r.applyType}</p><p class="text-danger small">${r.reason}</p>${btn}</div></div>`;
...
function filterTbl() {
    ...
    let lvFilter = document.getElementById('levelFilter').value; // ค่าจะเป็น "ม.1" หรือ "ม.4"
    ...
    let filteredData = allSt.filter(s => {
        ...
        // 🔥 กรองระดับชั้น (จุดที่แก้: เช็คเลข 1 หรือ 4 แทนการเช็คคำเป๊ะๆ)
        let matchLevel = true;
        if (lvFilter !== 'all') {
            // ดึงระดับชั้นมาเช็ค (ถ้าไม่มีข้อมูลให้เป็นค่าว่าง)
            let levelData = (s.level || '').toString(); 
            
            if (lvFilter === 'ม.1') {
                // ถ้าเลือก ม.1 -> ขอแค่มีเลข "1" อยู่ในข้อความ (รองรับทั้ง "ม.1" และ "มัธยมศึกษาปีที่ 1")
                matchLevel = levelData.includes('1');
            } else if (lvFilter === 'ม.4') {
                // ถ้าเลือก ม.4 -> ขอแค่มีเลข "4" อยู่ในข้อความ
    ...
    displayData.forEach(s => {
        ...
        let pName = (s.fullData[31]||'') + (s.fullData[32]||'') + ' ' + (s.fullData[33]||'');
        let pTel = s.fullData[36] || '-';
